...
\chg{EM section 2}{Besides shedding light on the average eye movement measures, the descriptive data is helpful for providing intuitions about the synthesized effect sizes. To this end, we transform the synthesized effect size for independent variable into its corresponding effect on fixation likelihood, fixation count and total dwell time. We first examine the relationship between individual study effect size correlations and the corresponding descriptive eye movement data for those studies reporting within conditions. Because fixation likelihood is a probability, we logit transform all fixation likelihood (FL) data, $logitFL = log(FL/1 - FL)$. We then compute the logit difference between conditions for each study, $logitDiff = logitFL_{condition 1} - logitFL_{condition 2}$ and regress the resulting logitDiff variable on effect size correlations expressed in fixation likelihood using a linear mixed model with a random intercept grouped by article to account for correlated errors. The model intercept is not significantly different from zero whereas the slope is, \input{tables/FLtoLogitModel}\unskip.  
Figure \ref{fig:em_figure} panel (D) illustrated the linear relationship between the logitDiff measure and effect size correlations with an increasing variance in logitDiff as effect sizes become larger.        
}