...
Thank you for the diligence and pointing out these new studies and the obvious inconsistency between the main and publication bias analyses. In accordance with our correspondence, we have adjusted the PET-PEESE and the public grants analyses with robust variance estimates and replaced the trim and fill analysis with the Top10 analysis. In the method we introduce the Top10 analysis and the computation of precision (section ``Publication bias'', line \lineref{publicaton-bias-method}). To avoid confusion by referencing concepts introduced later, we have moved the subsection so it appears after the subsections on the statistical analysis. Overall, this analysis has made the results stronger. The Top10 analysis results suggests that several visual factors now have larger effect sizes, while for the cognitive factors it suggests even larger downward adjustment. The main text was adjusted accordingly, mainly the Results section (lines \lineref{Top10-1}, \lineref{Top10-2}, \lineref{Top10-3}, \lineref{Top10-4} and \lineref{Top10-5}), and briefly in the Discussion section (line \lineref{Top10-discussion}).    
...
We have added more details on the results as well as the total number of effect sizes included in the study. 
...
As suggested we have added a title to demarcate the Limitations subsection (line \lineref{limitations}). However, to demarcate this section from the subsequent one, we have also added a title to the last section called ``Conclusion'' (line \lineref{conclusion}).  

Finally, related to the preceding point, we have pointed out lack of variability on ethnicity dimension as a potential limitation (line \lineref{limitations-diversity})  
