...
Thank you for the diligence and pointing out these new studies and the obvious inconsistency between the main and publication bias analyses. In accordance with our email correspondence, we have adjusted the PET-PEESE and the public grants analyses with robust variance estimates and replaced the trim and fill analysis with the Top10 analysis. In the method we introduce the Top10 analysis and the computation of precision (section ``Publication bias'', line \lineref{publicaton-bias-method}). To avoid confusion by referencing concepts introduced later, we have moved the publication bias subsection so it appears after the subsections on the statistical analysis. Overall, this analysis has made the results stronger. The Top10 analysis results suggests that several visual factors now have larger effect sizes, while for the cognitive factors it suggests larger downward adjustments. The main text was adjusted accordingly, mainly the Results section (lines \lineref{Top10-1}, \lineref{Top10-2}, \lineref{Top10-3}, \lineref{Top10-4} and \lineref{Top10-5}), and briefly in the Discussion section (line \lineref{Top10-discussion}).    
...
We have corrected this (line \lineref{independent}). 
...
We have browsed through recent articles in Psychological Bulletin and followed the prevalent style of reporting main results directly in the abstract. We thus report the main results of the psychometric meta-analysis and the Top10 analysis as well as the total number of effect sizes included in the study. 
...
We have revised the section and now provide a brief description of our expectations given the preceding review, and outline the implications in terms of theory and practice if the data supports our expectations (line \lineref{introend}).
...
The paper has now been proof read by a professional editor. Although numerous, by and large these were very small changes (e.g. missing commas), so we have not marked these changes.
...
We have not considered variables other than those listed in the method section. We have put a sentence stating this explicitly in the method (line \lineref{nonlisted-variables}).

We have now coded participant sample information following your recommendation (age, gender distribution, ethnicity, and country) and have placed a table summarizing this in the appendix. We have described this additional coding in the Method section (line \lineref{sample-information-method}), and briefly given some context in the main text, given this information (line \lineref{sample-information}).
...
As suggested we have added a title to demarcate the Limitations subsection (line \lineref{limitations}). However, to demarcate this section from the subsequent one, we have also added a title to the last section called ``Conclusion'' (line \lineref{conclusion}). We have added the reviewer point about English search terms and how it may have led to the exclusion of studies from non-western countries. We tie this point to the low variability in participant sample ethnicity as a potential limitation (line \lineref{limitations-diversity})  
...
We would like to thank all three reviewers for excellent and really constructive feedback. 
