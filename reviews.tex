...
We thank the reviewer for a concise summary and we are very happy that the reviewer likes our approach.  We are grateful for suggestions for improving the manuscript, we address each of them below. In the revised manuscript we now introduced line numbers and for each comment we refer to a line in the document where we introduced a major change. To facilitate the revision we also marked major changes in the main text with red color.
...
Thank you for the feedback, in particular it is good to know that our writing strikes a right balance.
...
\hs{I can take this one. I don't think we need to change anything in the text, we can address it here}\\
...
\jo{Revise into somewhat and cite Andi Glöckner a lot }

\hs{I can give this one a try\\
\begin{itemize}
    \item I wouldn't change the title, I agree it might be a bit too ambitious, but that's not that bad, I like it as it is :) 
    \item stressing that we are not concerned with causal effect of attention on choices is fine, that wasn't our goal at all - but probably worth discussing
    \item "do not really care overly much about attention and their predictions for it" - thats our implicit assumption part, I think this was supposed to be clear but we can be clearer there, sure
    \item "causal effects of attention on choice are tiny to not existing" - huh, not sure about this one, there are now quite a few studies claiming to show causal effects, but it could be that they were not done in a good way - this would require detailed knowledge of these studies :|
    \item "we can predict choice very well and close to choice reliability by our standard models" - this is true, but only sometimes, and for lab controlled studies where visual factors are removed - he is definitely not correct on this one - not sure if this would require any change in text, we can rebut it here
\end{itemize}
}
...
\jo{Get raw data to address point b) and address the rest in the discussion. Relatively easy}
\hs{salience would be a nice addition to our discussion, where we talk about small effect size - this could be a caveat (potentially we can check if effect is smaller in lottery type of tasks - I assume this is what Andi refers to the "simple standard paradigm"); c) potentially a fair point, we can point out this caveat, but I wouldn't stress it too much - I think Andy is biased with his lottery type of work, take for example sampling approach - from that viewpoint refixations are part of the process, you dont assume that with a single fixation you soaked in all the info}
...
\jo{easy} 
\hs{vision related - Jacob, yours? this could be few sentences somewhere in the discussion?} 
...
\jo{easy} 
\hs{choice bias is perhaps a wrong description, its still a bit unclear what is the underlying mechanism so we could keep on the safe side and call it differently (but what?) - we could also stick to it, as long as we mention gaze cascade somewhere where we introduce it; re Jekel 2018 I havent read it, you? do we have a spot for it in the discussion?}
...
\jo{Easy: cite Andi and his favorite literature}
\hs{are you familiar with this literature? willing to give this one a go?}
